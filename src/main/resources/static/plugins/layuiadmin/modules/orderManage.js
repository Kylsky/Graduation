/**
 *  @author   Caiwx
 *  @Explain  房源管理脚本
 */

layui.define(["form", "table", "element"], function (exports) {
    var $ = layui.$,
        table = layui.table
        , element = layui.element;
    element.render();

    /**
     * 初始化表格
     */
    var roomManageTable = table.render({
        elem: '#orderManageTable'
        , url: '/admin/orderManagePage'
        , page: true
        , limit: 10
        , height: 'full'
        , method: 'get'
        , request: {
            pageName: 'current' //页码的参数名称，默认：page
            , limitName: 'size' //每页数据量的参数名，默认：limit
        }
        , cols: [[
            {
                field: 'orderCode'
                , title: '订单号'
                , align: 'center'
                , event: 'detail'
                , width: 200
            },
            {
                field: 'rid'
                , title: '房间号'
                , align: 'center'
                , event: 'detail'
                , width: 100
            }
            , {
                field: 'price'
                , title: '价格'
                , align: 'center'
                , Width: 50
            }
            , {
                field: 'name'
                , title: '买家名称'
                , align: 'center'
                , Width: 100
            }
            , {
                field: 'phone'
                , title: '买家电话'
                , align: 'center'
                , Width: 200
            }
            , {
                    field: 'beginTime'
                        , title: '开始时间'
                        , align: 'center'
                        , Width: 220
                        , sort: true
                        , templet: (d) => {
                        return Base.formatDate(d.beginTime, 'yy/MM/dd HH:mm:ss');
                    }
                },
    {
        field: 'endTime'
            , title: '结束时间'
        , align: 'center'
        , Width: 220
        , sort: true
        , templet: (d) => {
        return Base.formatDate(d.endTime, 'yy/MM/dd HH:mm:ss');
    }
    },{
        field: 'status'
            , title: '订单状态'
            , align: 'center'
            , Width: 50
            , templet: (d) => {
            if (d.status== '1') {
                return '正常';
            } else if (d.status == '0') {
                return '关闭';
            } else {
                return d.status == '-1' ? '删除' : '异常';
            }
        }
    }

            ,
    {
                title: '操作'
                , width: 200
                , align: 'center'
                , fixed: 'right'
                , templet: (d) => {
                    var ableHtml = d.status == '0'
                        ? '<a class="layui-btn layui-btn-normal layui-btn-sm" lay-event="enable">激活</a>'
                        : '<a class="layui-btn layui-btn-danger layui-btn-sm" lay-event="disable">关闭</a>';
                    var detail = '<a lay-href="/admin/orderDetail?order_Code='+d.orderCode+'" class="layui-btn layui-bg-cyan layui-btn-sm" href="javascript:;" >详情</a>';
                    return detail+ableHtml;
                }
            }
        ]]
    });

    /**
     * 监听事件
     */
    $('button[data-type]').on('click', function () {
        var type = $(this).data('type');
        active[type] ? active[type].call(this) : '';
    });
    var active = {
        keyLike: function () {                          //关键词模糊搜索
            const content = $('#content');
            //执行重载
            table.reload('orderManageTable', {
                page: {
                    curr: 1 //重新从第 1 页开始
                }
                , where: {
                    content: content.val()
                }
            });
        },
        reload: function () {                           //重置加载页面
            $('#content').val("");
            table.reload('orderManageTable', {
                page: {
                    curr: 1 //重新从第 1 页开始
                }
                , where: {
                    content: $('#content').val()
                }
            });
        }
    };

    /**
     * 创建监听工具
     */
    table.on('tool(orderManageTable)', function (obj) {
        var data = obj.data;
        if (obj.event == 'detail') {         //点击查看内容详情
            layer.open({
                type: 0
                , title: '内容详情'
                , area: ['100px', '200px']
                , offset: 'auto'
                , shadeClose: true
                , id: 'layerDemo' + data.order_Code
                , content: '<div style="padding: 20px;">' + data.title + '</div>'
                , shade: 0.3
                , anim: 5
            });
        } else if (obj.event == 'audit') {                       //点击审核商家
            layer.open({
                type: 2
                , title: '审核'
                , shadeClose: true
                , shade: 0.2
                , area: ['396px', '505px']
                , offset: 'auto'
                , content: '/admin/toAudit?parentId=' + data.id + '&type=ROOM'
            });
        } else if (obj.event == 'lookHis') {                    //点击查看审核历史
            layer.open({
                type: 2
                , title: '审核历史'
                , shadeClose: true
                , shade: 0.2
                , area: ['742px', '350px']
                , offset: 'auto'
                , content: '/admin/toAuditHis?parentId=' + data.id + '&type=ROOM'
            });
        } else if (obj.event == 'disable') {
            layer.confirm('是否关闭该订单?', {icon: 3, title: '提示'}, function (index) {
                Base.ajax("/admin/opeOrder", "POST", {'id': data.id, 'status': '0'}, (res) => {
                    if (res.code === Base.status.success) {
                        layer.msg("操作成功", {icon: 6, time: 800});
                        setTimeout(() => {
                            layer.close(index);
                            $(".layui-icon-refresh").click();
                        }, 800)
                    } else {
                        layer.msg(res.msg, {icon: 5, time: 500});
                    }
                })
            });
        } else if (obj.event == 'enable') {
            layer.confirm('是否激活该订单?', {icon: 3, title: '提示'}, function (index) {
                Base.ajax("/admin/opeOrder", "POST", {'id': data.id, 'status': '1'}, (res) => {
                    if (res.code === Base.status.success) {
                        layer.msg("操作成功", {icon: 6, time: 800});
                        setTimeout(() => {
                            layer.close(index);
                            $(".layui-icon-refresh").click();
                        }, 800)
                    } else {
                        layer.msg(res.msg, {icon: 5, time: 500});
                    }
                })
            });
        }
    });

    /**
     * 创建排序规则
     */
    table.on('sort(roomManageTable)', function (obj) {
        var sort;
        if (obj.field == "createTime") {
            sort = "CREATE_TIME";
        } else if (obj.field == "beginTime") {
            sort = "BEGIN_TIME";
        } else if (obj.field == "endTime") {
            sort = "END_TIME";
        }
        roomManageTable.reload({
            initSort: obj
            , where: {
                sort: sort
                , order: obj.type
            }
        });
    });
    exports('orderManage', {});
});